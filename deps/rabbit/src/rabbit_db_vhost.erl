...
-spec create_if_missing(VHostName, Limits, Metadata) -> Ret when
      VHostName :: vhost:name(),
      Limits :: vhost:limits(),
      Metadata :: vhost:metadata(),
      Ret :: {existing | new, VHost},
      VHost :: vhost:vhost().
%% @doc Creates and writes a virtual host record if it doesn't exist already.
%%
...
create_if_missing(VHostName, Limits, Metadata)
  when is_binary(VHostName) andalso
       is_list(Limits) andalso
       is_map(Metadata) ->
    VHost = vhost:new(VHostName, Limits, Metadata),
    rabbit_db:run(
      #{mnesia => fun() -> create_if_missing_in_mnesia(VHostName, VHost) end}).

create_if_missing_in_mnesia(VHostName, VHost) ->
    rabbit_misc:execute_mnesia_transaction(
      fun() -> create_if_missing_in_mnesia_tx(VHostName, VHost) end).

create_if_missing_in_mnesia_tx(VHostName, VHost) ->
