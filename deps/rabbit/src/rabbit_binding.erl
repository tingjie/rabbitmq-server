    ...
    rabbit_khepri:transaction(
        fun () ->
                ...
                Fun = fun([?MODULE, durable_routes | Rest], Value) ->
                              ...
                              khepri_tx:put(SemiDurablePath, Value)
                      end,
...
-spec remove_for_source_in_mnesia(rabbit_types:binding_source(),
                                  boolean() | undefined) -> bindings().

-spec remove_for_source_in_mnesia(rabbit_types:binding_source() |
                                  rabbit_types:exchange()) ->
          bindings().

remove_for_source_in_mnesia(#exchange{name = SrcName} = SrcX) ->
    remove_for_source_in_mnesia(SrcName, should_index_table(SrcX));
remove_for_source_in_mnesia(SrcName) ->
    remove_for_source_in_mnesia(SrcName, undefined).

remove_for_source_in_mnesia(SrcName, ShouldIndexTable) ->
...
add_binding(Binding, _SrcDurable, false) ->
    ...
    {ok, _} = khepri_tx:put(ReversePath, sets:add_element(reverse_binding(Binding), RSet)).
...
sync_transient_route_in_khepri(Path0, Set) ->
    ...
    {ok, _} = khepri_tx:put(ReversePath, reverse_set_of_bindings(Set)).
